...
class pet(object):
    def __init__(self, tmax, tmin, rhmax, rhmin, u2, elevation, meteoindex,
                 solar=None, net=None):
        self.tmax = tmax.to_numpy()
        self.tmin = tmin.to_numpy()
        self.rhmax = rhmax.to_numpy()
        self.rhmin = rhmin.to_numpy()
        self.u2 = u2.to_numpy()

        self.elevation = elevation
        self.meteoindex = meteoindex
    ...
    def penman_monteith(u2, tmax, tmin, rhmin, rhmax, elevation, latitude,
                        meteoindex, solar=None, net=None, rs=69, ra=(208/self.u2)):
        """
        u2 = wind speed at 2m
        Rn = net solar radiation (MJ m-2 day-1)
        vpc = Slope of vapor pressure curve (kPa degC-1)
    ...
    def jensen_haise(tmax, tmin, meteoindex, latitude, solar, cr=0.02, tx=4):
        """
        Ra = extraterrestrial radiation (MJ m-2 day-1)
        lambd = latent heat of vaporization (MJ kg-1)
        Ta = air temperature (degrees C)
