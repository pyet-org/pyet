...
from spotpy.objectivefunctions import nashsutcliffe, kge, rsquared, bias

cm1 = 1 / 2.54  # centimeters in inches
figw_1c = 8.5 * cm1 # maximum width for 1 column
figw_2c = 17.5 * cm1 # maximum width for 2 columns

def scatter_1(ax, x, y, label="treatment", xlabel="obs", ylabel="sim", best_fit=True, veg_ws=None):
    ...
    ax.plot([-0.1,10], [-0.1,10], color="dodgerblue", alpha=0.7, linewidth="0.8")
    ax.axes.set_xticks(np.arange(0,10+2,2))
    ax.axes.set_yticks(np.arange(0,10+2,2))
    #ax.tick_params(pad=0.9)
    ax.set_xlim(-0.1, 10)
    ax.set_ylim(-0.1, 10)
    equation = "Best fit not calculated"
    if best_fit:    
        # From hydrostats (https://github.com/BYU-Hydroinformatics/Hydrostats/blob/master/hydrostats/visual.py)
    ...
    #
    ax.text(0.02, 0.9, f"{label}", color="k", zorder=10, transform=ax.transAxes)  
    ax.text(0.6, 0.04, "$Bias$ = "+str(round(bias(np.asarray(compare["y"]),np.asarray(compare["x"])),2)) + \
            "\n" + "$R^2$ = "+str(round(rsquared(np.asarray(compare["y"]),np.asarray(compare["x"])),2))+ \
            "\n" + "KGE = "+str(round(kge(np.asarray(compare["y"]),np.asarray(compare["x"])),2)), 
