...
def kpm(wind, elevation, latitude, rs=None, tmax=None, tmin=None,
        rhmax=None, rhmin=None, rh=None):
    """Returns evapotranspiration calculated with the Kimberly-Penman
    (Wright, 1982) method.
    ...
    Examples
    --------
    >>> pm = et.kpm(wind, elevation, latitude, rs=solar, tmax=tmax, \
    ...
    jd = day_of_year(ta.index)
    w = (0.4 + 0.14 * np.exp(-((jd - 173) / 58) ** 2)) + \
        (0.605 + 0.345 * np.exp(-((jd - 243) / 80) ** 2)) * wind

    den = (lambd * (dlt + gamma))
    num1 = (dlt * (rn - s_flux) / den)
    num2 = (gamma * (ea - ed) * w / den)
    pet = (num1 + num2)
    return pet
